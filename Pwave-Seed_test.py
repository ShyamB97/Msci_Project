...
def AnalysePwave(directory, label, CP=False):
    ...
    return pt.ErrorPlot([factors, value], label=label, legend=False, axis=True, y_error=error, x_axis="relative P-wave amplitudes", y_axis="$A_{T}$")


def AnalyseCPwave(direct, direct_CP, label):
    datas = GenerateDataFrame(direct, False)
    datas_CP = GenerateDataFrame(direct_CP, True)
    ...
    for i in range(len(factors)):
        ...
        data_amp_CP = MergeData(data_amp_CP)
        C_T = kin.Scalar_TP(kin.Vector_3(data_amp[3]), kin.Vector_3(data_amp[4]), kin.Vector_3(data_amp[1]))
